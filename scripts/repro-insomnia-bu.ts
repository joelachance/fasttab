 ...
 *
 * ## What this proves
 * FastTab sends Browser Use to insomniacookies.com for delivery. The agent hits
 * reCAPTCHA (or login) before a guest cart is ready. Production SMS then falls back
 * to an internal draft cart — users never see the live Browser Use session.
 *
 ...
 * Optional: BROWSER_USE_MODEL, BROWSER_USE_CART_TIMEOUT_MS (default 270000), BROWSER_USE_MAX_COST_USD
 *
 * ## Run
 *   bun run repro:insomnia-bu
 *   bun run repro:insomnia-bu -- --phone +15551234567
 *   bun run repro:insomnia-bu -- --address "560 20th St, San Francisco, CA"
 *   bun run repro:insomnia-bu -- --dry-run   # print official-site cart prompt only
 *
 * ## Expected result
 * - liveUrl like https://live.browser-use.com/session/<uuid>
 * - blockers containing "reCAPTCHA" (or login/payment) and status "blocked" or "draft"
 * - Often zero cart items when blocked before menu add
 *
 * ## Production SMS path (Option B)
 * Same env on Vercel + text: "Insomnia Cookies delivery to 560 20th St, San Francisco"
 * with FOODRUN_INSOMNIA_CATALOG_CART=false. Cart build job uses the same prompts as this script.
 */
...
import { BrowserUseModule, buildCartPrompt } from "../src/modules/browser-use/index.js";
...
function usage(): never {
  console.error(`Usage: bun run repro:insomnia-bu [--phone E164] [--address "…"] [--dry-run] [--timeout SECONDS]
...
function parseArgs(argv: string[]) {
  ...
  return { phone, address, dryRun, timeoutSeconds };
}
...
async function main(): Promise<void> {
  ...
  if (
    ...
    console.error(
      "  Warning: using AGENTPHONE_PHONE_NUMBER as delivery phone (set FOODRUN_DELIVERY_PHONE for customer E.164).",
    );
  }
  ...
  console.error(`  Timeout: ${Math.round(timeoutMs / 1000)}s`);
  console.error("  Running buildCart (official site → Grubhub fallback)…");

  const browser = new BrowserUseModule(env);
  const startedAt = Date.now();

  const result = await browser.buildCart(criteria, restaurant, {
    keepAlive: true,
  ...
  if (!recaptchaHit && result.output.status === "blocked") {
    console.error(
      "\nNote: blockers did not mention reCAPTCHA this run (login/payment/other). Replay live URL or re-run.",
    );
  }
